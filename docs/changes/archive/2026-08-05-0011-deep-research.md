...
Premise invalidated by change 0012: the core deliverable (ResearchOrchestrator goroutine fan-out as 'precursor to the subagent dispatch layer') now duplicates the shipped, hardened subagent runtime — parallel spawn_agent batches with width capping, slot yielding, and context management. Building a second bespoke orchestration mechanism would be a regression. The still-valuable remainder (web search/fetch tools, query diversification, cited synthesis) is a much smaller change that drives the EXISTING fan-out instead of building its own; to be re-captured as a new stub.
