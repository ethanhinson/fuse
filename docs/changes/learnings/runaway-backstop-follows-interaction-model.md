...
created: 2026-08-06
updated: 2026-08-17
topics: [architecture, agents, safety, ux, permissions]
---
...
**A backstop that trips must also say how to un-trip (change 0067).** A trip that is permanent for
the session is a second, quieter version of the same bug: the limit is now correctly calibrated but
still terminal, so one false positive costs the whole run. Give every trip an exit whose shape
follows the interaction model — **interactive** gets exactly one recovery prompt (approve → reset
the counters and let the pending call proceed; reject → fall back to per-call asks, and never
re-prompt), **headless** gets one *structured* stop rather than a crash. The "exactly once" is
load-bearing in both directions: without it, the recovery prompt becomes the new prompt-fatigue
source it was meant to relieve.

## War story

### 2026-08-17 (#67, PR #71)

Auto mode's escalation valve tripped 8 times in one day of session logs, and a tripped valve was
**permanent for the session** — the run was over regardless of whether the trip was justified.
0067 made it recoverable on both postures: an interactive trip issues one recovery approval
(counters reset under the valve mutex, safe across `CloneForChild`, with a `promptedOnce` guard so
a rejection degrades to per-call asks silently); a headless trip returns a typed
`ErrAutoModePaused` that keeps tool-result pairing intact and collapses to a partial result the way
`ErrMaxTurns` already did, instead of tearing down the run. Same principle as the 0038 turn cap,
one layer down: the limit was fine, its *terminality* was the defect.
